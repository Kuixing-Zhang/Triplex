...
rootpath=/share/home/zhangkx/Triplex/CUTTAG_DNMT1
Datapath=/share/home/zhangkx/Triplex/CUTTAG_DNMT1/Raw/X101SC23121304-Z01-J010/01.RawData
...
ls *R1.clean.fq.gz | while read id; do bowtie2 --end-to-end --very-sensitive --no-mixed --no-discordant -p 10 -x /share/home/zhangkx/Triplex/bowtie2_index/hg38_spike/hg38_spike -1 $id -2 ${id%_R1*}_R2.clean.fq.gz | samtools sort -@ 10 -o - > ${rootpath}/align/${id%_R1*}.sorted.bam; done

cd ${rootpath}/align
ls *.bam | xargs -i samtools index {}
ls *.bam | while read id; do samtools flagstat $id > ${rootpath}/stat/$(basename $id ".bam").stat ; done

ls *sorted.bam | while read id ; do samtools view -b -L "/share/home/zhangkx/Triplex/Ref_genome/hg38/hg38.bed" $id > ${rootpath}/part/${id%%.*}.hg38.bam; done
ls *sorted.bam | while read id ; do samtools view -b -L "/share/home/zhangkx/Triplex/Ref_genome/hg38/CnT_SpikeIn.bed" $id > ${rootpath}/part/${id%%.*}.spike.bam; done
...
cat ${rootpath}/peak/DNMT1_DMSO_BQQ_q001_peaks.narrowPeak | bedtools intersect -v -a - -b "/share/home/zhangkx/Triplex/Ref_genome/hg38-blacklist.v2.bed" > ${rootpath}/peak/DNMT1_DMSO_BQQ_q001_noBlacklist.bed
