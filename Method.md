...
Circular dichroism spectra was recorded on 2.2 µM samples of triplex and random DNA duplex in CD buffer (10 mM sodium acetate at pH 5.5, 10 mM MgCl2). The spectra were obtained at 20℃ on Applied Photophysis Chirascan with a 10 mm quartz spectrophotometer cell using the following parameters: 200–340 nm wavelengths with continuous scanning mode, 0.5 nm data pitch, standard sensitivity (±200 mdeg), 4 s digital integration time, 2 nm bandwidth. Data were an average of 3 scans.



## Cell growth and viability assay

For cell growth before and after DDX3X knock down with probe treatment , 2.5x 10^4  HeLa and A549 cells were seeded in 24-well plates and number of cells were counted at the series time (0, 4, 8, 24, 48, 72 hours). For cell viability with probe treatment, 10^5  HeLa and A549 cells were seeded in 24-well plates and cells were counted after 4 hour.
...
### ChIP-seq peaks HDNA region define and retio plot
...
HeLa cell,BQQ 5um, 4h and 24h

统一策略：对各生物学重复合并，然后对所有处理合并，加入spike in normalized call peak，在peak上看原处理的相应位置碱基覆盖度
...
ls *.unique10.sorted.bam | while read id; do /share/apps/anaconda3/envs/python3/bin/picard -Xmx4g  MarkDuplicates REMOVE_DUPLICATES=true Input=${id} OUTPUT=${id%.*}.rmdup.bam METRICS_FILE=${name%.*}.metrics; done
#bamcouverage
ls *.rmdup.bam | while read id; do bamCoverage --normalizeUsing CPM -b ${id} -o ./${id%.*}.bw --binSize 10 -p 6; done
